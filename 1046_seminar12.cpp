...
//scriere in fisier
//vector de pointeri - reference type

class WriterCSV {
public :
	virtual void scrieCSV(ofstream &out) = 0;
};

class Cafenea : public WriterCSV{
	string nume;
	int nrProduse;
	float* preturi;
	TipCafea* tipuri;
...
public:
	...
	void scrieCSV(ofstream &out) {
		out <<endl<< this->nume << "," << this->nrProduse << "," << this->nrClientiZi << "," << this->anInfiintare << "," << this->TVA<<",";
	}
	float operator()() {
		float suma = 0;
...
public:
	...
	void scrieCSV(ofstream& out) {
		Cafenea::scrieCSV(out);
		out << this->numeFranciza << "," << this->pretFranciza;
...
void main() {
	...
	WriterCSV** vector = new WriterCSV * [5];
	vector[0] = &c1;
	vector[1] = &cf2;
	vector[2] = &c3;
	vector[3] = &cf1;
	vector[4] = &c2;
	ofstream fisier("Cafenele.csv", ios::app);
	for (int i = 0; i < 5; i++) {
		vector[i]->scrieCSV(fisier);
	}
	fisier.close();
